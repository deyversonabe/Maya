...
Versao atual do projeto: 3.3.17  
...
# Atualizacao 3.3.17 - Nara valida documentos e CRM separado da Agenda

## O que foi ajustado

- O gerador passou a aplicar um modelo automatico por empresa, identificando tipo documental, modelo de tabela, visual e regra de exibicao.
- A previa do orcamento agora mostra um checklist da Nara antes da exportacao.
- A exportacao em PDF bloqueia documentos com numero `ORC` indevido, valor original, acrescimo, percentual, prazo, validade, garantia, condicoes comerciais/de execucao ou texto generico de preenchimento.
- A Nara valida se a tabela de itens fecha exatamente com o valor global informado na empresa selecionada.
- O CRM agora lista somente clientes com historico humano real.
- Contatos criados sem conversa ficam salvos na Agenda telefonica e nao poluem o CRM.
- Ao salvar um contato sem historico, o sistema sincroniza com a Agenda sem duplicar por nome, telefone ou e-mail.
- Mantidas todas as melhorias anteriores no mesmo pacote.
...
Build executado com sucesso na versao 3.3.17.

## Arquivo final desta versao

```text
outputs/orcaflow-studio-ai-3.3.17-20260810-121828.zip
```

---

# Atualizacao 3.3.16 - DNA visual real por empresa nos orcamentos

## O que foi ajustado

- O gerador de propostas agora usa modelos de tabela diferentes por perfil de empresa.
- A Eng. Lider usa quadro tecnico por referencia, composicao, base e valor final.
- A Orlovic usa matriz corporativa por frente de melhoria, solucao contemplada e investimento.
- A Construir usa etapas livres de obra, sem aparencia de planilha.
- A AD Solucoes usa quadro analitico de registros, atividade analisada, base e valor final.
- A Power usa pacotes operacionais com entrega e valor final.
- A H&H usa blocos de producao/evento.
- A Eletro Lider permanece com estilo de planilha comercial de loja.
- A ordem dos itens pode variar por perfil para reduzir semelhanca visual entre documentos.
- Foram removidas colunas visiveis de valor original, percentual, acrescimo e valor unitario nos modelos de proposta.
- O prompt da IA foi reforcado para respeitar o modelo de tabela de cada empresa e nunca explicar rateio, margem ou preco de origem.
- A pre-visualizacao em tela e o PDF baixado foram atualizados no mesmo padrao para evitar divergencia.

## Validacao

- `npm run build` executado com sucesso na versao 3.3.16.

## Arquivo final desta versao

```text
outputs/orcaflow-studio-ai-3.3.16-20260810-083000.zip
```
